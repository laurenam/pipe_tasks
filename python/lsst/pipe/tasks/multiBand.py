...
class MergeDetectionConfig(MergeSourcesConfig):
...
class MergeDetectionsTask(MergeSourcesTask):
    ...
    ConfigClass = MergeDetectionConfig
    ...
    def mergeCatalogs(self, catalogs, patchRef):
        ...
        # Convert distance to tract cooridante
        ...
        ordered_catalogs = [catalogs[band] for band in self.config.priorityList if band in catalogs.keys()]
        ordered_bands = [band for band in self.config.priorityList if band in catalogs.keys()]

        mergedList = self.merged.getMergedSourceCatalog(ordered_catalogs, ordered_bands, peakDistance,
